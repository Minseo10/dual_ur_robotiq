...
//  rightArm.execute(my_plan);
  ...
  right_goal_pose = setGeomPose(-0.65, 0.0, 1.35, 0.0, 0.0, 1.0, 0.0);
  left_goal_pose = setGeomPose(-0.65, 0.0, 1.35, 0.0, 0.0, 1.0, 0.0);
...
//  right_goal_pose.position.z += 0.1;
  ...
  left_goal_pose.position.z = 0.315;
  leftArm.setApproximateJointValueTarget(left_goal_pose, "left_gripper_tool0");
  success = (leftArm.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  visual_tools.trigger();
  ...
  visual_tools.trigger();
  visual_tools.prompt("1-3. Press 'next' to draw");
  left_goal_pose.position.y = -0.2;

  linear_path.push_back(left_goal_pose);
  leftArm.computeCartesianPath(linear_path, 0.001, 0.0, trajectory);
  my_plan.trajectory_ = trajectory;
  leftArm.execute(my_plan);
  linear_path.clear();
  left_goal_pose.position.x += 0.2;
  leftArm.computeCartesianPath(linear_path, 0.001, 0.0, trajectory);
  my_plan.trajectory_ = trajectory;
  leftArm.execute(my_plan);
  linear_path.clear();
...
//  right_goal_pose = setGeomPose(-0.64, 0.08, 1.65, 0.0, -0.7071068, 0.0, 0.7071068);
//  rightArm.setApproximateJointValueTarget(right_goal_pose, "right_gripper_tool0");
//  success = (rightArm.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
////
...
//  rightArm.move();
//
//  visual_tools.prompt("Press 'next' to save transformation matrix at pose 1");
...
//  right_goal_pose = setGeomPose(-0.6399, 0.175, 1.574, 0.3321, -0.6490, -0.2810, 0.6240);
//  rightArm.setApproximateJointValueTarget(right_goal_pose, "right_gripper_tool0");
//  success = (rightArm.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
////
////  // repeat until it succeeds
